...
resource "vault_policy" "seaweedfs_s3_provisioner" {
  name = "seaweedfs-s3-provisioner"

  policy = <<-EOT
    path "seaweedfs/data/s3/bindings/*" {
      capabilities = ["create", "read", "update"]
    }
  EOT
}

resource "vault_approle_auth_backend_role" "seaweedfs_s3_provisioner" {
  backend        = vault_auth_backend.approle.path
  role_name      = "seaweedfs-s3-provisioner"
  token_policies = [vault_policy.seaweedfs_s3_provisioner.name]
